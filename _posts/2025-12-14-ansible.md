...
read_time: true
show_date: true
title: "Ansible로 Debian 서버 배포 준비하기"
date: 2025-12-14
...
이번 글에서 정리한 흐름은 크게 네 가지다.

1. **쿡북 스타일 Ansible 프로젝트 구조** 만들기
2. **Ubuntu → Debian** 구조로 컨트롤/관리 노드 구분
3. **직접 만든 플레이북**으로 코드 clone, venv, requirements 설치 자동화
4. **Ansible Galaxy nginx role**을 붙여 reverse proxy vhost까지 구성

---

### 다음에 이어서 할 수 있는 것들

* `roles/quizai/`를 만들어서 `git pull`, `migrate`, `collectstatic`, `systemd` 재시작까지 묶기
* 민감 정보는 `group_vars`/Vault로 분리하기
* 배포 검증용 헬스체크 태스크 추가하기

최종적으로는 playbook은 얇게, role은 두껍게 가는 구조가 유지보수에 유리하다.
지금은 프로젝트 수가 많지 않아 과한 느낌이 있을 수 있지만, 서버가 늘어날수록 이런 구조화의 가치가 커진다.
